...
- **Suíte automatizada:** POC 1–6 com 209 verificações Godot (208 quando o adapter está no ar e o POC 3 valida a resposta real em vez do fallback, ver `docs/TESTES.md`) + 16 testes do adapter Python — todas PASS (`tests/run_all.sh`).
...
- **LOCAL** — suíte automatizada headless: POC 1 (77), POC 2 (32: inclui iluminação em camadas, letreiros e câmera de segurança), POC 3 (25 com adapter no ar / 26 com adapter desligado), POC 4 (21), POC 5 (23) e POC 6 (30) verificações + 16 testes do adapter Python; todas PASS (`tests/run_all.sh`). Desempenho: 60 FPS (vsync) em iGPU AMD Radeon Graphics @1280×720, inclusive após o detalhamento da sala (248 objetos, 15 luzes). Pendências: hardware real, empacotamento e playthrough humano.
